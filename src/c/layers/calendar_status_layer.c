...
void calendar_status_layer_create(Layer *parent_layer, GRect frame) {
  ...
  s_bt_disconnect_bitmap =
      gbitmap_create_with_resource(RESOURCE_ID_IMAGE_BT_DISCONNECT);
  ...
  s_calendar_month_layer =
      text_layer_create(GRect(0, -MONTH_FONT_OFFSET, w, 25));
  ...
  text_layer_set_font(s_calendar_month_layer,
  ...
  layer_add_child(s_calendar_status_layer,
                  bitmap_layer_get_layer(s_mute_bitmap_layer));
  layer_add_child(s_calendar_status_layer,
                  bitmap_layer_get_layer(s_bt_bitmap_layer));
  layer_add_child(s_calendar_status_layer,
                  bitmap_layer_get_layer(s_bt_disconnect_bitmap_layer));
  layer_add_child(s_calendar_status_layer,
                  text_layer_get_layer(s_calendar_month_layer));
  battery_layer_create(s_calendar_status_layer,
                       GRect(w - BATTERY_W - PADDING, 1, BATTERY_W, BATTERY_H));
...
void bluetooth_icons_refresh(bool connected) {
  ...
  layer_set_hidden(bitmap_layer_get_layer(s_bt_disconnect_bitmap_layer),
                   !show_bt_disconnect);
}
...
void status_icons_refresh() {
  ...
  bitmap_layer_move_frame(s_bt_bitmap_layer,
                          show_qt ? ICON_SLOT_2 : ICON_SLOT_1);
  bitmap_layer_move_frame(s_bt_disconnect_bitmap_layer,
                          show_qt ? ICON_SLOT_2 : ICON_SLOT_1);
