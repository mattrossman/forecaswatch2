...
void time_layer_create(Layer *parent_layer, GRect frame) {
  ...
  layer_add_child(text_layer_get_layer(s_container_layer),
                  text_layer_get_layer(s_time_layer));
  layer_add_child(text_layer_get_layer(s_time_layer),
                  text_layer_get_layer(s_am_pm_layer));
...
void time_layer_tick() {
  ...
  text_layer_move_frame(
      s_time_layer,
      GRect(0, 0, bounds.size.w, bounds.size.h)); // Reset for size calculation
  ...
  text_layer_move_frame(s_time_layer,
                        GRect(text_left, text_top, content_w, time_size.h));
  if (g_config->show_am_pm)
    text_layer_move_frame(
        s_am_pm_layer, GRect(time_size.w, MT_TIME - MT_AM_PM, 30, time_size.h));
...
void time_layer_refresh() {
  ...
  text_layer_set_text_color(
      s_time_layer, PBL_IF_COLOR_ELSE(g_config->color_time, GColorWhite));
