...
static void velo_update_proc(Layer *layer, GContext *ctx) {
...

  {
    ...
    text_layer_set_font(s_advice_text_layer,
    ...
    graphics_fill_rect(ctx,
                       GRect(bounds.origin.x, h - 3,
                             (((double)bounds.size.w) / 100.0) * (advice % 100),
                             h + FONT_OFFSET - 6),
                       1, GCornersAll);
...
void velo_layer_create(Layer *parent_layer, GRect frame) {
  ...
  }
  {
    s_advice_text_layer = text_layer_create(
        GRect(0, 1 * box_h - FONT_OFFSET - 3, w, box_h + FONT_OFFSET));
