...
static void city_layer_refresh() {
  ...
  int w = bounds.size.w - frame_curr_temp.size.w - frame_sun_event.size.w -
          MARGIN * 4;
...
static void current_temp_layer_refresh() {
  ...
  text_layer_move_frame(
      ...
      GRect(0, 0, 100, 100)); // Make it big so content doesn't get clipped
  GSize size = text_layer_get_content_size(s_current_temp_layer);
  text_layer_move_frame(s_current_temp_layer,
                        GRect(MARGIN, -FONT_18_OFFSET, size.w, size.h));
...
static void sun_event_layer_refresh() {
  ...
  text_layer_move_frame(s_next_sun_event_layer,
                        GRect(bounds.size.w - MARGIN - ARROW_W - size.w,
                              -FONT_14_OFFSET, size.w + ARROW_W, size.h));
  frame_sun_event = GRect(bounds.size.w - MARGIN - ARROW_W - size.w,
                          -FONT_14_OFFSET, size.w + ARROW_W + MARGIN, size.h);
}
...
static void weather_status_layer_init(GRect bounds) {
  ...
  // Current temperature
  s_current_temp_layer =
      text_layer_create(GRect(MARGIN, -FONT_18_OFFSET, 40, 25));
  ...
  text_layer_set_font(s_current_temp_layer,
                      fonts_get_system_font(FONT_KEY_GOTHIC_18));

  // City where weather was fetched
  s_city_layer = text_layer_create(
      GRect(w / 2 - CITY_INIT_WIDTH / 2, -FONT_14_OFFSET, CITY_INIT_WIDTH, 25));
  ...
  s_next_sun_event_layer =
      text_layer_create(GRect(w - MARGIN - 6 - 40, 4 - FONT_18_OFFSET, 40, 25));
  ...
  text_layer_set_font(s_next_sun_event_layer,
...
void weather_status_layer_create(Layer *parent_layer, GRect frame) {
  ...
  layer_add_child(s_weather_status_layer,
                  text_layer_get_layer(s_current_temp_layer));
  layer_add_child(s_weather_status_layer,
                  text_layer_get_layer(s_next_sun_event_layer));
